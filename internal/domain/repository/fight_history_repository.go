...
type FightHistoryRepository struct {
	pool *pgxpool.Pool
}

// GetLast5Fights retrieves the last 5 fights for a fighter
func (r *FightHistoryRepository) GetLast5Fights(ctx context.Context, fighterID int) ([]models.FightHistory, error) {
	query := `
	...
	rows, err := r.pool.Query(ctx, query, fighterID)
	if err != nil {
...
func (r *FightHistoryRepository) CreateFightHistory(ctx context.Context, fight *models.FightHistory) error {
	query := `
